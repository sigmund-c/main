...
A session started by the user when the user types `test INDEX`. Shows the question on each flashcard in the deck at `INDEX` sequentially to the user, and requires the user to input an answer before proceeding to the next flashcard.
