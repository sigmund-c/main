...
The following sequence diagram illustrates how the `LogicToUiManager` can be used to modify the `MainWindow`. This diagram is also an extension of the sequence diagram shown in <<test-session>> (_Figure 9_).
...
The Test Feature makes use of 2 classes in the `Model` component: `TestSession` and `TestResult`, as well as a `Result` enums class as only one of 3 result outcomes are allowed for a flashcard - wrong, correct or skipped.
...
The above commands can only be triggered when certain conditions are met. The following table provides a summary of all the `Command` classes related to the test feature. These classes are part of the *Logic* component, and can be found in the *testsession* package.

.Summary of Test Feature Commands
[width="%",cols="5%,<5%,<25%,<60%,<5%",options="header",]
|=======================================================================
