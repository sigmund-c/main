...
a/B) Architecture Diagram C) Sequence Diagram A) Object Diagram`. If the file path is invalid, the middle part of the card will be blank and an image will not be shown. +
 +
...
a/B) Architecture Diagram C) Sequence Diagram A) Object Diagram`.
...
* searching by keywords from the title of a `Deck` or from the `Question` from a `Flashcard`.
* filtering by tags from the `Tag(s)` associated with a `Deck`
